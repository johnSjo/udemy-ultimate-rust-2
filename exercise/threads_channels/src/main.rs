...
fn main() {
    ...
    let (tboo, rboo) = channel::unbounded();
    let rboo_b = rboo.clone();
    let kid_a = thread::spawn(move || {
        for msg in rboo {
            println!("Kid A received: {}", msg);
            sleep_ms(10);
        }
    });
    let kid_b = thread::spawn(move || {
        for msg in rboo_b {
            println!("Kid B received: {}", msg);
            sleep_ms(10);
        }
    });

    let val = vec!["asdf", "asfd", "34rt", "bh65"];

    for msg in val {
        println!("Sending: {}", msg);
        tboo.send(msg).unwrap();
    }
